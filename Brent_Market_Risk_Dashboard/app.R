...
# ------------------------------------------------------------
# 1. Load pre‑computed dataset from RDS (NO source analysis script)
# ------------------------------------------------------------

shiny_data <- readRDS("results/shiny_data.rds")

# Unpack all objects into global environment, names keep unchanged
oil <- shiny_data$oil
risk_summary <- shiny_data$risk_summary
stress <- shiny_data$stress
ewma_data <- shiny_data$ewma_data
ml_results <- shiny_data$ml_results
importance_df <- shiny_data$importance_df
rf_model <- shiny_data$rf_model
hedge_ratio <- shiny_data$hedge_ratio
VaR_95_unhedged <- shiny_data$VaR_95_unhedged
VaR_95_hedged <- shiny_data$VaR_95_hedged
VaR_reduction <- shiny_data$VaR_reduction
unhedged_vol <- shiny_data$unhedged_vol
hedged_vol <- shiny_data$hedged_vol
vol_reduction <- shiny_data$vol_reduction
